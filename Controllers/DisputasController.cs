    ...
    public class DisputasController : ControllerBase
    {
        ...
        public async Task<IActionResult> AtaqueComArmaAsync(Disputa d)
        {
            try
            {
                ...
                if (dano > 0)
                    oponente.PontosVida = oponente.Defesa - (int)dano;
                ...
                dados.AppendFormat("Atacante: {0}.", oponente.Nome);
        ...
        public async Task<IActionResult> AtaqueComHabilidadeAsync(Disputa d)
        {
            try
            {
                Personagem atacante = await _context.TB_PERSONAGENS.Include(p => p.PersonagemHabilidades).ThenInclude(PhysicalFile => PhysicalFile.Habilidade).FirstOrDefaultAsync(p => p.Id == d.AtacanteId);

                Personagem oponente = await _context.TB_PERSONAGENS.FirstOrDefaultAsync(p => p.Id == d.OponenteId);

                PersonagemHabilidade ph = await _context.TB_PERSONAGENS_HABILIDADES.Include(p => p.Habilidade).FirstOrDefaultAsync(phBusca => phBusca.HabilidadeId == d.HabilidadeId && phBusca.PersonagemId == d.AtacanteId);
                return Ok(d);
            }
            catch (System.Exception ex)
            {
        ...
        public async Task<IActionResult> DisputaEmGrupoAsync(Disputa d)
        {
            try
            {
                ...
                while (qtdPersonagensVivos > 1)
                {
                    ...
                    {
                        string resultadoFinal = $"{atacante.Nome.ToUpper()} é CAMPEÃO com {atacante.PontosVida} pontos de vida!";
